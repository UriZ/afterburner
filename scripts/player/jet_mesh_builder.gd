...
static func build_player_jet() -> Node3D:
	...
	var mat_fuselage := _make_mat(Color(0.92, 0.91, 0.88), 0.0, 0.85)
	var mat_wing := _make_mat(Color(0.82, 0.81, 0.78), 0.0, 0.85)
	var mat_tail_fin := _make_mat(Color(0.85, 0.84, 0.82), 0.05, 0.65)
	var mat_nacelle := _make_mat(Color(0.55, 0.54, 0.52), 0.1, 0.5)
	var mat_nozzle := _make_mat(Color(0.12, 0.12, 0.12), 0.2, 0.3)
	var mat_canopy := _make_mat(Color(0.08, 0.35, 0.85, 0.75), 0.05, 0.2)
	...
	mat_flame.shading_mode = BaseMaterial3D.SHADING_MODE_PER_PIXEL
	...
	# Nose cone — cylinder with top_radius=0 (Godot cone)
	...
	_add_part(root, "LeftNacelle", nacelle_mesh, mat_nacelle,
		Vector3(-0.35, -0.1, 0.8), Vector3(90, 0, 0))
	_add_part(root, "RightNacelle", nacelle_mesh, mat_nacelle,
		Vector3(0.35, -0.1, 0.8), Vector3(90, 0, 0))

	# Engine nozzles — dark rings
	...
	_add_part(root, "LeftNozzle", nozzle_mesh, mat_nozzle,
		Vector3(-0.35, -0.1, 1.55), Vector3(90, 0, 0))
	_add_part(root, "RightNozzle", nozzle_mesh, mat_nozzle,
		Vector3(0.35, -0.1, 1.55), Vector3(90, 0, 0))

	# Canopy — squashed sphere on top of fuselage
	...
	_add_part(root, "LeftHStab", hstab_mesh, mat_wing,
		Vector3(-0.55, 0.0, 1.1), Vector3(0, 0, -5))
	...
	_add_part(root, "LeftFlame", flame_mesh, mat_flame,
		Vector3(-0.35, -0.1, 1.8), Vector3(90, 0, 0))
