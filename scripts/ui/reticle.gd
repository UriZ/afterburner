...
func _draw_lock_markers() -> void:
	...
	for enemy in _weapon_manager.locked_enemies:
		...
		_draw_lock_box(screen_pos)


func _draw_lock_box(pos: Vector2) -> void:
	## Red bracket-style lock marker around an enemy position.
	var s := LOCK_BOX_SIZE
	var arm := s * 0.4
	# Top-left
	draw_line(pos + Vector2(-s, -s), pos + Vector2(-s + arm, -s), COLOR_LOCKED, LOCK_STROKE)
	draw_line(pos + Vector2(-s, -s), pos + Vector2(-s, -s + arm), COLOR_LOCKED, LOCK_STROKE)
	# Top-right
	draw_line(pos + Vector2(s, -s), pos + Vector2(s - arm, -s), COLOR_LOCKED, LOCK_STROKE)
	draw_line(pos + Vector2(s, -s), pos + Vector2(s, -s + arm), COLOR_LOCKED, LOCK_STROKE)
	# Bottom-left
